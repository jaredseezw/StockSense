...
const showRawTickerOption =
  ...
  useEffect(() => {
    ...
    const unsubPositions = onSnapshot(positionsRef, (snap) => {
      setPositions(snap.docs.map((d) => {
        const data = d.data();
        return {
          ticker: d.id,
          ...data,
          // Guard against already-stored floating-point dust (e.g. an old
          // buy that landed as 0.19999999999999996 shares before the
          // rounding fix existed) — always show at most 2 decimal places,
          // no matter what's actually sitting in Firestore.
          shares: Math.round((data.shares || 0) * 100) / 100,
        };
      }));
    });
                    ...
                    <div className="card portfolioPositionsCard">
                      ...
                      {sample.map((h) => (
                        <div className="table" key={h.ticker}>
                          ...
                          <span>{Number(h.shares).toFixed(2).replace(/\.00$/, "")}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                    ...
                    <>
                      ...
                      {txLog.map((t) => (
                        <div className="table" key={t.id} style={{ gridTemplateColumns: "1fr 1fr 1fr 1fr 1fr" }}>
                          ...
                          <b>{t.ticker}</b>
                          <span>{Number(t.shares).toFixed(2).replace(/\.?0+$/, "")} shares</span>
